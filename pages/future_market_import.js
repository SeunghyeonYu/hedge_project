                                    ...
                                    <div>
                                        <div className="info">
                                                    ...
                                                    <table className="table my-2">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td></td>
                                                                <td>{store.storedFetchData.forwardPurchase}</td>
                                                            </tr>
                                                            <tr>
                                            ...
                                            <div className="summary col-md-6">
                                                ...
                                                <p>[선물환시장을 이용한 헤징]</p>
                                                <p>선물환 매입은 현재 시점에서 미리 정한 환율로 미래시점에 외환을 매입하는 것을 의미합니다.</p>
                                                <p>즉, 선물환 매입을 하게 되면 미래의 특정시점에 일정가격으로 달러를 사고 이를 원화로 교환하게 됨으로써
                                                    미래 시점에 '달러가 나가는 상황'이 '원화가 나가는 상황'으로 바뀌어 환변동 위험에 노출되는 것을 막아줍니다.</p>
                                                <p>따라서, 귀사가 {store.storedWishYear}년 뒤에 ${store.storedWishMoney}를 지급하게 되는 경우, 선물환 매입을 하게 되면 환율 변동과 상관없이 안정적으로 ￦{store.storedFetchData.sumOfForward}를 지급할 수 있게 됩니다.</p>
                                                <br/>

                                                <p>[단기금융시장을 이용한 헤징]</p>
                                                <p>이와 같은 포지션으로는, 달러화 대출 + 현물환 매입 + 원화 차입을 통해서도 동일한 효과를 얻을 수 있는데
                                                이 경우에는 달러화의 대출 이자율을 고려하여 미래의 시점에 지급하게 될 달러와 같은 금액이 되도록 현재시점에서 대출을 하고
                                                대출을 한 달러화의 금액만큼 현물환을 매입(현재시점에서 달러를 사고 원화로 교환)하고
                                                    그 원화만큼의 금액을 현재시점에서 차입하게 되면, 차입이자율을 고려한 미래의 원화가치만큼이 안정적으로 나가게 됩니다.</p>
                                                <p>하지만, 이 금액은 ￦{store.storedFetchData.sumOfMarket}으로 선물환시장을 이용한 방법보다  {store.storedFetchData.resultHedge}만큼 더 많이 지급하게 되므로
                                                    전자에 기술한 선물환 매입 방식이 귀사에 더 유리한 전략이라고 볼 수 있습니다.</p>
